    ...
    public class SeparValuesParser<TItem> : ISeparValuesParser<TItem> where TItem : class, new()
    {
        public TItem Parse(string str)
        {
            ...
            if (!String.IsNullOrWhiteSpace(str))
            {
                ...
                SeparatorAttribute separatorAttr = type.GetTypeInfo().GetCustomAttribute<SeparatorAttribute>();
                if (separatorAttr != null)
                {
                    item = new TItem();
                    var parts = str.Split(separatorAttr.Separator);
                    PropertyInfo[] props = type.GetProperties().Where(p => p.IsDefined(typeof(IndexAttribute))).ToArray();
