...
module.exports = {
    ...
    renderSelect(props) {
        ...
        return (
            <DropdownButton bsSize={props.bsSize} title={selectedTitle || 'select'} id={`dropdown-basic-${props.name}`} onSelect={props.onSelect}>
                {items.map((item, index) =>
                    (<MenuItem key={index} eventKey={item} active={props.input.value === item.id}>{item.title}</MenuItem>)
                )}
            </DropdownButton>
        )
    }
